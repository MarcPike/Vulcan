...
namespace Vulcan.WebApi2.Controllers
{
    ...
    public class TeamController : BaseController
    {
        ...
        public async Task<JsonResult> RefreshTeamCompanyNames(string application, string userId, string teamId)
        {
            ...
            try
            {
                await Task.Run(() =>
                {
                    ...
                    if ((crmUser.IsAdmin) || (crmUser.UserType == CrmUserType.Manager) && (crmUser.Teams.Any(x => x.Id == userId)))
                    {
        ...
        public async Task<JsonResult> GetTeamActivities(string application, string userId, string teamId, string fromDateString, string toDateString)
        {
            dynamic result = new ExpandoObject();
            result.Success = false;
            var statusCode = CheckToken(application, userId);
        ...
        public async Task<JsonResult> RefreshTeamCompaniesFromCompanyGroups(string application, string userId, string teamId)
        {
            dynamic result = new ExpandoObject();
            result.Success = false;
            var statusCode = CheckToken(application, userId);
