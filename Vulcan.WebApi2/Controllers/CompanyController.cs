        ...
        public async Task<JsonResult> GetMyTeamCompanies(string application, string userId)
        {
            ...
            try
            {
                await Task.Run(() =>
                {
                    ...
                    result.Companies = companies.Where(x => x.IsActive).ToList();
                    result.Success = true;
                });
        ...
        public async Task<JsonResult> GetFullCompany(string application, string userId, string coid, string companyId)
        {
            ...
            try
            {
                await Task.Run(() =>
                {
                    ...
                    var companySearchResult = query.GetForId(companyRef.SqlId);
                    if (companySearchResult == null) throw new Exception("Company no longer exists in iMetal");

                    result.CompanyRef = companyRef;
                    result.Company = new { companySearchResult.CustomerGroupCode, companySearchResult.CustomerGroupDescription, companySearchResult.Branch, companySearchResult.Code, companySearchResult.Name, companySearchResult.Addresses, companySearchResult.Telephone, companySearchResult.FastDial, companySearchResult.Fax, companySearchResult.Email };
