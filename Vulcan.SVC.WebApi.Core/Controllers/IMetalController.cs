...
using AndOrValue = DAL.Vulcan.Mongo.Core.DocClass.Quotes.AndOrValue;
using CrmQuoteItem = DAL.Vulcan.Mongo.Core.DocClass.Quotes.CrmQuoteItem;
using IHelperApplication = DAL.Vulcan.Mongo.Core.Helpers.IHelperApplication;
using IHelperCompany = DAL.Vulcan.Mongo.Core.Helpers.IHelperCompany;
using IHelperFile = DAL.Vulcan.Mongo.Core.Helpers.IHelperFile;
using IHelperUser = DAL.Vulcan.Mongo.Core.Helpers.IHelperUser;
using JsonResult = Microsoft.AspNetCore.Mvc.JsonResult;
using MadeUpCost = DAL.Vulcan.Mongo.Core.DocClass.Quotes.MadeUpCost;
using OrderQuantity = DAL.Vulcan.Mongo.Core.Quotes.OrderQuantity;
using OsirisDocumentListForPortal = DAL.Vulcan.Mongo.Core.Models.OsirisDocumentListForPortal;
using OsirisDocumentListModel = DAL.Vulcan.Mongo.Core.Models.OsirisDocumentListModel;
using ProductBucket = DAL.Vulcan.Mongo.Core.DocClass.Quotes.ProductBucket;
using StockItemsQueryModel = DAL.Vulcan.Mongo.Core.Models.StockItemsQueryModel;
...
namespace Vulcan.SVC.WebApi.Core.Controllers
{
    ...
    {
        private readonly DAL.Vulcan.Mongo.Core.Helpers.IHelperApplication _helperApplication;
        private readonly DAL.Vulcan.Mongo.Core.Helpers.IHelperCompany _helperCompany;
        private readonly DAL.Vulcan.Mongo.Core.Helpers.IHelperFile _helperFile;
        private readonly DAL.iMetal.Core.Helpers.IHelperCurrencyForIMetal _helperCurrencyForIMetal;
        ...
        [HttpGet]
        [AllowAnonymous]
        [Route("imetal/GetCacheInfo/{application}/{userId}")]
        public async Task<JsonResult> GetCacheInfo(string application, string userId)
        {
            ...
            try
            {
                await Task.Run(() =>
                {
                    ...
                    result.CacheInfo = new CacheSettingsModel();
                    result.Success = true;
        ...
        public async Task<JsonResult> GetProductMastersForCoid(string application, string userId, string coid)
        {
            ...
            try
            {
                await Task.Run(() =>
                {
                    ...
                    result.CacheId = cacheId.ToString();
        ...
        public async Task<JsonResult> LoadNewSalesGroupsFromIMetal(string application, string userId)
        {
            ...
            try
            {
                await Task.Run(() =>
                {
                    ...
                    var rep = new DAL.Vulcan.Mongo.Base.Repository.RepositoryBase<DAL.Vulcan.Mongo.DocClass.CRM.SalesGroup>();
                    ...
                    {
                        var salesGroups = SalesGroupQuery.ExecuteAsync(coid).Result;
        ...
        public async Task<JsonResult> GetPartNumbersForCoid(string application, string userId, string coid)
        {
            ...
            try
            {
                ...
                {
                    ThrowExceptionForBadToken(statusCode);
                    var partNumbers = new DAL.Vulcan.Mongo.Base.Core.Repository.RepositoryBase<CrmQuoteItem>().AsQueryable().Where(x => x.Coid == coid && x.PartNumber != string.Empty).GroupBy(info => info.PartNumber)
        ...
        public async Task<JsonResult> GetProductsWithNoInventory(string application, string userId, string coid, bool forceReload)
        {
            ...
            try
            {
                await Task.Run(() =>
                {
                    ...
                    var stockItemsProductIds = StockItemsCache.Helper
            ...
            try
            {
                await Task.Run(() =>
                {
...

                    {
                        ...
                        var queryOrders = OrdersQuery.ExecuteAsync(coid).Result.ToList();

                        queryOrders = queryOrders.Where(x => companyCodes.Contains(x.CompanyCode)).ToList();

                        queryOrders = queryOrders.Where(x => x.OrderDueDate >= dueDateMin && x.OrderDueDate <= dueDateMax).ToList();
                        results = queryOrders.Where(x => x.OrderSaleTypeCode == "ORD").ToList();
        ...
        public async Task<JsonResult> GetOrderDetailsForOrderNumber(string application, string userId, string coid, int orderNumber)
        {
            ...
            try
            {
                ...
                {
                    ThrowExceptionForBadToken(statusCode);
                    result.OrderDetails = OrdersQuery.GetProjectionForOrderNumber(coid, orderNumber);
                    result.Success = true;
                });
        ...
        private static void ProcessOrdersAccumulateTotals(List<OrdersQuery> orders, string coid, List<OrderListModel> model, List<CompanyListModel> companyTotals, List<SalesPersonListModel> salesPersonTotals)
        {
        ...
        */
        ...
        public async Task<JsonResult> GetWareHousesForCoid(string application, string userId, string coid)
        {
            ...
            try
            {
                ...
                {
                    ThrowExceptionForBadToken(statusCode);
                    result.WareHouses = await WarehouseQuery.ExecuteAsync(coid);
                    result.Success = true;
                });
            }
            catch (Exception e)
        ...
        public async Task<JsonResult> GetStockItemsForProductId(string coid, int productId)
        {
            ...
            try
            {
                await Task.Run(() =>
                {
                    ...
                    var stockItemQuery = StockItemsQuery.GetForId(coid, productId);
        ...
        public async Task<JsonResult> GetIncomingPurchaseOrderItemsForProductId(string coid, int productId)
        {
            ...
            try
            {
                await Task.Run(() =>
                {
                    ...
                    result.PurchaseOrderItems = PurchaseOrderItemsQuery.GetForId(coid, productId).ToList();
        ...
        public async Task<JsonResult> GetStockItems([FromBody] DAL.Vulcan.Mongo.Models.StockItemsQueryModel model)
        {
            dynamic result = new ExpandoObject();
            result.Success = false;
            ...
            try
            {
                await Task.Run(() =>
                {
                    ...
                    var helper = StockItemsCache.Helper;
                    var filter = helper.FilterBuilder.
                        Where(x => x.Coid == model.Coid && x.CacheId == cacheId);
            ...
            IQueryable<StockItemsCondensed> FilterForProductBucket(IQueryable<StockItemsCondensed> query)
            {
                ...
                {
                    var bucket = new DAL.Vulcan.Mongo.Base.Core.Repository.RepositoryBase<ProductBucket>().Find(model.ProductBucketId);
        ...
        public async Task<JsonResult> GetStockItemQueryModel(string application, string userId)
        {
            ...
            try
            {
                await Task.Run(() =>
                {
                    ...
                    result.StockItemQueryModel = new DAL.Vulcan.Mongo.Models.StockItemsQueryModel(application, crmUser.User.Id);
                    result.Success = true;
        ...
        public async Task<JsonResult> GetPurchaseOrderItems([FromBody] PurchaseOrderItemsQueryModel model)
        {
            ...
            try
            {
                await Task.Run(() =>
                {
                    ...
                    var helper = PurchaseOrderItemsCache.Helper;
        ...
        public async Task<JsonResult> GetProductCategoryListForCoid(string application, string userId, string coid)
        {
            ...
            try
            {
                ...
                {
                    ThrowExceptionForBadToken(statusCode);
                    result.ProductCategoryList = ProductMastersQuery.GetAllProductCategories(coid);

                    result.StockItemQueryModel = new DAL.Vulcan.Mongo.Core.Models.StockItemsQueryModel();
                    result.Success = true;
                });
            }
            catch (Exception e)
        ...
        public async Task<JsonResult> GetProductConditionListForCoid(string application, string userId, string coid)
        {
            ...
            try
            {
                ...
                {
                    ThrowExceptionForBadToken(statusCode);
                    result.ProductConditionList = ProductMastersQuery.GetAllProductConditions(coid);
