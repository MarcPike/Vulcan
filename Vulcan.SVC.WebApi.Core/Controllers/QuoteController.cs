...
using RPT.HtmlTemplateLibrary.Core;
    ...
    public class QuoteController : BaseController
    {
        ...
        public async Task<JsonResult> GetIMetalExportErrorInfo(string application, string userId, string quoteId)
        {
            ...
            try
            {
                ...
                await Task.Run(() =>
                {
                    var rep = new RepositoryBase<CrmQuote>();
                    var crmQuote = rep.Find(quoteId);
                    result.QuoteExportAttemptResultsModel = new QuoteExportAttemptResultsModel(crmQuote);
        ...
        public async Task<JsonResult> GetNewLostQuoteModel(string application, string userId, string quoteId)
        {
            ...
            try
            {
                ...
                await Task.Run(() =>
                {
                    ...
                    result.Competitors = lostQuoteModelValues.Competitors;
                    result.LostReasons = lostQuoteModelValues.LostReasons;
        ...
        public async Task<JsonResult> MoveQuoteItemsToNewQuote([FromBody] QuoteCopyModel model)
        {
            ...
            try
            {
                ...
                await Task.Run(() =>
                {
                    var newQuote = _helperQuote.MoveQuoteItemsToNewQuote(model);
                    result.NewQuoteId = newQuote.QuoteId;
                    result.NewId = newQuote.Id.ToString();

                    var origQuote = CrmQuote.Helper.FindById(model.QuoteId);

                    result.QuoteModel = new QuoteModel(model.Application, crmUser.UserId, origQuote);
                    result.QuoteMiniModel = new QuoteMiniModel(origQuote, PipelineStatus.Draft);
            ...
            string companyId)
        {
            ...
            try
            {
                await Task.Run(() =>
                {
                    ...
                    var iMetalCompany = CompanyQuery.GetForId(coid, company.SqlId).Result;
                    if (iMetalCompany == null)
                        throw new Exception($"Company was not found in iMetal. Coid: {coid} SqlId: {company.SqlId}");
                    quoteModel.PaymentTerm = iMetalCompany.PaymentTerms;

                    result.QuoteModel = quoteModel;
                    //result.PaymentTermList = CompanyQuery.GetPaymentTerms(coid).Result.ToList();
                    //result.FreightTermList = CompanyQuery.GetFreightTerms(coid).Result.ToList();
        ...
        public async Task<JsonResult> GetNewQuoteTestPieceModel(string application, string userId)
        {
            ...
            try
            {
                await Task.Run(() =>
                {
                    ...
                    result.Lengths = result.CreateNewQuoteTestPieceModel.OrderQuantity.Lengths;
                    result.Success = true;
                });
            }
            catch (Exception e)
            ...
            GetAddressesForCompanyFromIMetal(string coid, string companyId)
        {
            ...
            {
                Id = Guid.NewGuid(),
                AddressLine1 = iMetalCompany.PrimaryAddress.Address,
                AddressLine2 = string.Empty,
                City = iMetalCompany.PrimaryAddress.Town,
                StateProvince = iMetalCompany.PrimaryAddress.County,
                PostalCode = iMetalCompany.PrimaryAddress.PostCode,
                Country = iMetalCompany.PrimaryAddress.CountryName,
                Type = AddressType.Primary
            };

            var otherAddresses = new List<Address>();
            otherAddresses.AddRange(iMetalCompany.Addresses.Select(x => new Address
            {
                Id = Guid.NewGuid(),
            ...
            string barOrTube, string productCondition, string resourceTypeName)
        {
            ...
            try
            {
                await Task.Run(() =>
                {
                    ...
                    var productMasterResults = ProductMastersQuery.ExecuteAsync(coid).Result;

                    var matchingProducts = productMasterResults.Where(x =>
                    ...
                    var productMasterList = new List<ProductMaster>();

                    foreach (var p in matchingProducts)
                        productMasterList.Add(new ProductMaster
                        {
                    ...
                    result.ProductMasterList = productMasterList;
                    result.Success = true;
                });
            }
            catch (Exception e)
            ...
            string tagNumber)
        {
            ...
            try
            {
                ...
                {
                    ThrowExceptionForBadToken(statusCode);
                    var currentStockItem = StockItemsQuery.GetForTag(coid, tagNumber);

                    if (currentStockItem == null) throw new Exception("MachinedPartFromCacheValue not found");


                    var allStockItems = StockItemsQuery.GetForId(coid, currentStockItem.StockItemId);
            ...
            try
            {
                await Task.Run(() =>
                {
                    ...
                    foreach (ResourceType perType in Enum.GetValues(typeof(ResourceType)))
                        resourceTypeList.Add(perType.ToString());
        ...
        public async Task<JsonResult> GetProductBucketListForCoid(string application, string userId, string coid)
        {
            ...
            try
            {
                await Task.Run(() =>
                {
                    ...
                    if (buckets.Any())
                        result.ProductBuckets =
                            buckets.Select(x => new {Id = x.Id.ToString(), x.Name}).OrderBy(x => x.Name);
                    else
