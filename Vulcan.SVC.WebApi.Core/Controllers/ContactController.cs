...
namespace Vulcan.SVC.WebApi.Core.Controllers
{
    ...
    public class ContactController : BaseController
    {
        ...
        public async Task<JsonResult> SetReportsTo(string application, string userId, string companyId,
            string contactId,
            string reportsToId)
        {
            dynamic result = new ExpandoObject();
            ...
            try
            {
                await Task.Run(() =>
                {
                    ...
                    else
                    {
                        ...
                        var reportsToResults = _helperContact.SetReportsTo(application, userId, company,
                            contact.AsContactRef(), reportsTo);
        ...
        public async Task<JsonResult> GetAllAvailableContactsForCompany(string application, string userId,
            string companyId)
        {
            dynamic result = new ExpandoObject();
            result.Success = false;
            ...
            try
            {
                await Task.Run(() =>
                {
                    ThrowExceptionForBadToken(statusCode);
                    result.ContactList =
                        _helperContact.GetAllAvailableContactsForCompany(application, userId, companyId);
                    result.Success = true;
                });
            }
            catch (Exception e)
        ...
        public async Task<JsonResult> GetAllAvailableContactsForProspect(string application, string userId,
            string prospectId)
        {
            dynamic result = new ExpandoObject();
            result.Success = false;
            ...
            try
            {
                await Task.Run(() =>
                {
                    ThrowExceptionForBadToken(statusCode);
                    result.ContactList =
                        _helperContact.GetAllAvailableContactsForProspect(application, userId, prospectId);
                    result.Success = true;
                });
            }
            catch (Exception e)
        ...
        public async Task<JsonResult> GetAllAvailableContactsForCompanyGroup(string application, string userId,
        ...
        public async Task<JsonResult> GetContactsForUser(string application, string userId)
        {
            ...
            try
            {
                await Task.Run(() =>
                {
                    ...
                    result.ContactList = crmUser.Contacts
                        .Select(x => new ContactModel(application, userId, x.AsContact()))
                        .OrderBy(x => x.Person.FirstName).ThenBy(x => x.Person.LastName).ToList();
                    result.Success = true;
                });
            }
            catch (Exception e)
        ...
        public async Task<JsonResult> GetContactsForTeam(string application, string userId, string teamId)
        {
            ...
            try
            {
                await Task.Run(() =>
                {
                    ...
                    var contacts = repQuote.AsQueryable().Where(x => x.Team.Id == teamId && x.Contact != null)
                        .Select(x => x.Contact).Distinct().OrderBy(x => x.FirstName)
