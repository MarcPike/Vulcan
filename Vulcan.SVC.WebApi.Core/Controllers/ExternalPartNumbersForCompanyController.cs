    ...
    public class ExternalPartNumbersForCompanyController : BaseController
    {
        ...
        public async Task<JsonResult> GetExternalPartNumbersForCompany(string coid, string companyNameStartsWith)
        {
            ...
            try
            {
                await Task.Run(() =>
                {
                    result.PartNumbers = DAL.QNG.Reader.GetProducts.GetProductsFor(coid, companyNameStartsWith);
                    result.Success = true;
                });
            }
            catch (Exception ex)
        ...
        public async Task<JsonResult> GetQuotesForCompany(DateTime begDate, DateTime endDate, string companyId, bool showExpired)
        {
            dynamic result = new ExpandoObject();
            var statusCode = HttpStatusCode.OK;
            result.Success = false;
            ...
            try
            {
                await Task.Run(() =>
                {
                    var quotePipeline = helperQuote.GetQuotesPipelineForCompany(begDate, endDate, companyId, showExpired);
