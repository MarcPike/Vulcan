    ...
    private  bool ActionContactsSearch(Action action, string searchFor)
        {
            foreach (var contact in action.Contacts.ConvertAll(x => x.AsContact()))
            {
                ...
                foreach (var email in contact.Emails.ConvertAll(x=>x.AsEmail()))
                {
                    if (ContactEmailSearch(action, searchFor, email)) return true;
                }
        ...
        private  bool ContactEmailSearch(Action action, string searchFor, Email email)
        {
            if (email.Subject.ToUpper().Contains(searchFor))
            {
                Results.Add("Action.Contact.Email.Subject",action.AsActionRef());
            }
            foreach (var searchTag in email.SearchTags)
            {
                if (searchTag.ToUpper().Contains(searchFor))
                {
                    Results.Add("Action.Contact.Email.SearchTag", action.AsActionRef());
                }
