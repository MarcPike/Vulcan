...
namespace DAL.Vulcan.Mongo.DocClass.Companies
{
    public class Company: BaseDocument
    {
        ...
        {
            var query = new QueryCompany(Location.GetCoid());

            var companySearchResult = query.GetForId(SqlId);
        ...
        {
            var context = ContextFactory.GetCompanyContextForCoid(Location.GetCoid());
            var externalContacts = context.Contact.Where(x => x.CompanyId == SqlId).ToList();
            var crmContacts = Contacts.Select(x => x.ToBaseDocument()).ToList();
            
            foreach (var contact in externalContacts.ToList())
            {
                if (crmContacts.All(x => x.SqlId != contact.Id))
                {
        ...
        public List<Address> GetAllAddressesFromIMetal()
        {
            ...
            return companySearchResult.Addresses.Select(x => new Address(x, AddressType.Shipping)).ToList();
        }
    }
}
