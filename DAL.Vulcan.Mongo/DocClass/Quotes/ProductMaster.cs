...
namespace DAL.Vulcan.Mongo.DocClass.Quotes
{
    public class ProductMaster
    {
        ...
        public static (ProductMaster ProductMaster, StockItemsAdvancedQuery StockItem) FromStockId(string coid, int stockItemId)
        {
            var query = StockItemsAdvancedQuery.AsQueryable(coid);
            var stockItem = query.FirstOrDefault(x => x.StockItemId == stockItemId);
            ...
            var productMaster = new ProductMaster()
            {
                ...
                FactorForKilograms = stockItem.FactorForKilograms,
        ...
        {
            var query = StockItemsAdvancedQuery.AsQueryable(coid);
            var result = query.First(x => x.ProductCode == productCode);
            ...
            FactorForKilograms = result.FactorForKilograms;
        ...
        public ProductMaster(string coid, int productId)
        {
            ...
            var result = query.FirstOrDefault(x => x.ProductId == productId);
        ...
        public ProductMasterAdvancedQuery GetProductMasterFull()
        {
            return ProductMasterAdvancedQuery.AsQueryable(Coid).SingleOrDefault(x=>x.ProductId == ProductId);
        }
        ...
        public string GetLongDescription(RequiredQuantity requiredQuantity, string partNumber, string partSpecification)
        {
            ...
            var productMasterFull = ProductMasterAdvancedQuery.AsQueryable(Coid).SingleOrDefault(x => x.ProductId == ProductId);
