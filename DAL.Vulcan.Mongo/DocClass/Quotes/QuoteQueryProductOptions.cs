...
namespace DAL.Vulcan.Mongo.DocClass.Quotes
{
    public class QuoteQueryProductOptions
    {
        ...
        public bool QuoteItemPassed(CrmQuoteItem crmQuoteItem)
        {
            ...
            var context = new StockItemsContext();

            var query = ProductMasterAdvancedQuery.AsQueryable(startingProduct.Coid,context);
            var stockItems = query.Where(x =>
                x.ProductId == startingProduct.ProductId || x.ProductId == finishedProduct.ProductId).ToList();

            var stockItemStart = stockItems.FirstOrDefault(x => x.ProductId == startingProduct.ProductId);
            var stockItemFinish = stockItems.FirstOrDefault(x => x.ProductId == finishedProduct.ProductId);
