...
namespace DAL.Vulcan.Mongo.Models
{
    public class QuoteModel
    {
        ...
        public string DefaultExcelTemplateId { get; set; } = string.Empty;
        ...
        public QuoteModel(string application, string userId, CrmQuote quote)
        {
            try
            {
                if (quote.Company != null)
                {
                    ...
                    var companyDefaults =
                        CompanyDefaults.GetCompanyDefaults(quote.Coid, company, quote.Team.Name.Contains("Norway"));
                    if (!String.IsNullOrEmpty(companyDefaults.ExcelTemplateId))
                    {
                        DefaultExcelTemplateId = companyDefaults.ExcelTemplateId;
                    }
            ...
            FileAttachments = Base.FileAttachment.FileAttachmentsVulcan.GetAllAttachmentsForDocument(quote).Select(x=> new FileAttachmentModel(x)).ToList() ?? new List<FileAttachmentModel>();
