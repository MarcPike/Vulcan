...
namespace DAL.Vulcan.Mongo.Core.DocClass.Companies
{
    public class CompanyDefaults: BaseDocument
    {
        ...
        public static void ApplyCompanyDefaultsToQuote(CrmQuote quote, bool isMoving = false)
        {
            ...
            if (!string.IsNullOrEmpty(companyDefaults.DisplayCurrency) && (string.IsNullOrEmpty(quote.DisplayCurrency)))
            {
        ...
        public static CompanyDefaults GetCompanyDefaults(string coid, Company company, bool isNorway)
        {
            ...
            var iMetalCompany = CompanyQuery.GetForId(coid,company.SqlId, true).Result; ;

            if (iMetalCompany == null)
            {
                throw new Exception($"This company was not found in iMetal. Coid: {coid} SqlId: {company.SqlId}");
            }
            ...
            if (companyDefaults == null)
            {
                ...
                //{
                //    var shippingAddress = company.Addresses.LastOrDefault(x => x.Type == AddressType.Shipping) ??
                //                          company.Addresses.LastOrDefault(x => x.Type == AddressType.Other) ??
                //                          company.Addresses.LastOrDefault(x => x.Type == AddressType.Primary) ??
                //                          company.Addresses.Last();
                //    companyDefaults.ShippingAddressId = shippingAddress.Id;
                //}
