...
namespace DAL.Vulcan.Mongo.Core.DocClass.CRM
{
    public class Team: BaseDocument
    {
        ...
        {
            HelperUser _helperUser = new HelperUser();
            ...
            Locations.Location.Helper.Upsert(location);
        }
        ...
        public void RefreshCompanyNames()
        {
            ...
            foreach (var companyRef in Companies.ToList())
            {
                ...
                var companySearchResult = CompanyQuery.GetForCode(coid, companyRef.Code).Result;
                ...
                if (companySearchResult.Name != companyRef.Name)
                {
