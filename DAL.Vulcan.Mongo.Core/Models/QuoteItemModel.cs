...
namespace DAL.Vulcan.Mongo.Core.Models
{
    public class QuoteItemModel
    {
        ...
        public QuoteItemModel(string application, string userId, CrmQuoteItem item)
        {
            ...
            QuotePriceModel = (item.QuotePrice == null)
                ? null
                : new QuotePriceModel(item.CalculateQuotePriceModel.Application, item.CalculateQuotePriceModel.UserId,
                    item.QuotePrice);
            ...
            CustomerUom = item.CustomerUom.ToString();
            ...
            CrozCalcItem = (item.CrozCalcItem != null)
                ? new CrozCalcItemModel(application, userId, item.CrozCalcItem)
                : null;
            ...
            if (IsMachinedPart)
            {
                CustomerUom = "PerPiece";
            }

            FileAttachments =
                Base.Core.FileAttachment.FileAttachmentsVulcan.GetAllAttachmentsForDocument(item)
                    .Select(x => new FileAttachmentModel(x)).ToList() ?? new List<FileAttachmentModel>();
