...
namespace DAL.Vulcan.Mongo.Core.Helpers
{
    public class QuoteQueryProductOptionsHelper
    {
        ...
        private readonly List<ProductMastersQuery> _allTeamProducts = new List<ProductMastersQuery>();
        private readonly List<ProductMastersQuery> _allSystemProducts = new List<ProductMastersQuery>();
        ...
        private void AllSystemFullProducts(string coid)
        {
            ...
            {
                _coidList.Add(coid);
                var query = ProductMastersQuery.ExecuteAsync(coid).Result;
