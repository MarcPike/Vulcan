...
namespace DAL.Vulcan.Mongo.Core.Helpers
{
    public class HelperQuote : HelperBase, IHelperQuote
    {
        ...
        public CrmQuote MoveQuote(CrmQuote quote, Team newTeam, CrmUser newSalesPerson, string newCompanyId)
        {
            ...
            CompanyDefaults.ApplyCompanyDefaultsToQuote(quote, true);
            ...
            DateTime endDate, bool forTeam)
        {
            ...
            //var quotes = quoteRep.AsQueryable().Where(x => x.ReportDate >= begDate && x.ReportDate <= endDate);

            //var problemQuote = quotes.SingleOrDefault(x => x.QuoteId == 7373);

            //quotes = forTeam ? quotes.Where(x => x.Team.Id == teamRef.Id) : quotes.Where(x => x.SalesPerson.Id == crmUser.Id.ToString() && x.Team.Id == teamRef.Id);

            var quotes = collection.Find(filter).ToList();

            //problemQuote = quotes.SingleOrDefault(x => x.QuoteId == 7373);
            ...
            GetNewCalculateQuotePriceModelForStockItem(QuoteStockItemModel model)
        {
            ...
            var kurfInchesPerCut = (decimal) 0.25;
        ...
        {
            var machinePartCost = new MachinedPartCostPriceValue(model.Coid, model.ProductMaster.ProductId,
                model.Pieces, model.DisplayCurrency);
            ...
            GetNewCalculateQuotePriceModelForMadeUpCost(MadeUpCostModel model)
        {
            ...
            if (madeUpCost == null)
            {
                if (model.TheoWeight < 0) throw new Exception("MadeupCost theoretical weight is less than zero");
            ...
            string sourceQuoteItemId)
        {
            ...
            sourceQuote = CrmQuote.Helper.FindById(sourceQuote.Id);
        ...
        public CrmQuote CreateLinkedQuote(string application, string userId, string sourceQuoteId, string forCompanyId)
        {
            ...
            var paymentTerms = CompanyQuery.GetForId(sourceQuote.Coid, destCompany.SqlId).Result.PaymentTerms;

            result.PaymentTerm = paymentTerms;
        ...
        private CrmQuoteItem SaveQuoteItem(QuoteItemModel model, string displayCurrency)
        {
            ...
            try
            {
                quoteItem = new RepositoryBase<CrmQuoteItem>().AsQueryable()
                                .SingleOrDefault(x => x.Id == ObjectId.Parse(model.Id)) ??
...

            {
                try
                {
                    model.QuotePriceModel.CustomerUom ??= model.CustomerUom;
