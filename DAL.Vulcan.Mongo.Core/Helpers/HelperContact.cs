...
namespace DAL.Vulcan.Mongo.Core.Helpers
{
    public class HelperContact : HelperBase, IHelperContact
    {
        ...
        public void RemoveContact(string application, string userId, string contactId)
        {
            ...
            RemoveContactFromCrmUser();
            RemoveFromAllCompanies();
            RemoveFromAllProspects();

            //Contact.Helper.DeleteOne(contactId);

            void RemoveContactFromCrmUser()
            {
            ...
            void RemoveFromAllCompanies()
            {
                foreach (var company in contact.Companies.Select(x => x.AsCompany()).ToList())
                {
                    ...
                    foreach (var removeContact in removeMe)
                    {
                        ...
                        var companyDefaults =
                            CompanyDefaults.GetCompanyDefaults(company.Location.GetCoid(), company, isNorway);
            ...
            void RemoveFromAllProspects()
            {
                foreach (var prospect in contact.Prospects.Select(x => x.AsProspect()).ToList())
                {
                    ...
                    foreach (var removeContact in removeMe) prospect.Contacts.Remove(contactRef);
        ...
        public (ContactModel contactModel, CompanyContactTree companyContactTree) SetReportsTo(string application,
            string userId, Company company, ContactRef contact, ContactRef reportsTo)
        {
