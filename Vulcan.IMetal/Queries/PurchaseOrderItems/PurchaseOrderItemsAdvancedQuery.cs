...
namespace Vulcan.IMetal.Queries.PurchaseOrderItems
{
    ...
    public class PurchaseOrderItemsAdvancedQuery
    {
        ...
        {
            context = context ?? ContextFactory.GetPurchaseOrdersContextForCoid(coid);
            ...
            var stockItemQuery = (
                ...
                select new PurchaseOrderItemsAdvancedQuery()
                {
                    ...
                    Density = (
                         ((pc.Dim1TypeId == dimTypeIdForDensity) ? (dim.Dim1 ?? (decimal) 0) : (decimal) 0)) +
                         ((pc.Dim2TypeId == dimTypeIdForDensity) ? (dim.Dim2 ?? (decimal) 0) : (decimal) 0) +
                         ((pc.Dim3TypeId == dimTypeIdForDensity) ? (dim.Dim3 ?? (decimal) 0) : (decimal) 0) +
                         ((pc.Dim4TypeId == dimTypeIdForDensity) ? (dim.Dim4 ?? (decimal) 0) : (decimal) 0) +
                         ((pc.Dim5TypeId == dimTypeIdForDensity) ? (dim.Dim5 ?? (decimal) 0) : (decimal) 0),
            ...
            //stockItemQuery = stockItemQuery.Where(x => x.InternalStatusId == 1).AsQueryable();
