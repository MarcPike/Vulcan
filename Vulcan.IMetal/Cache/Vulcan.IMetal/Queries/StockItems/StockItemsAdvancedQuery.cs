...
namespace Vulcan.IMetal.Queries.StockItems
{
    ...
    public class StockItemsAdvancedQuery
    {
        ...
        public static List<string> GetProductCategories(string coid, StockItemsContext context = null)
        {
            context ??= ContextFactory.GetStockItemsContextForCoid(coid);

            return context.ProductCategory.Where(x => x.Status == "A" && !String.IsNullOrEmpty(x.Category) && !x.Category.StartsWith("MC")).Select(x => x.Category).OrderBy(x=>x).ToList();
        }

        public static List<string> GetProductConditions(string coid, StockItemsContext context = null)
        {
            context ??= ContextFactory.GetStockItemsContextForCoid(coid);

            return context.Product.Where(x => x.Status == "A" && !String.IsNullOrEmpty(x.SpecificationValue2)).Select(x => x.SpecificationValue2).Distinct().OrderBy(x => x).ToList();
        }
