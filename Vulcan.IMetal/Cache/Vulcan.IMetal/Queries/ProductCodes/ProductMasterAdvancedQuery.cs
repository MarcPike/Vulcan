...
namespace Vulcan.IMetal.Queries.ProductCodes
{
    ...
    public class ProductMasterAdvancedQuery
    {
        ...
        public static List<string> AllProductCodesOnly(string coid, StockItemsContext context = null)
        {
            context ??= ContextFactory.GetStockItemsContextForCoid(coid);
            return context.Product.Where(x=>x.Status == "A").Select(x=>x.Code).OrderBy(x => x).ToList();
        ...
        public static IQueryable<ProductMasterAdvancedQuery> AsQueryable(string coid, StockItemsContext context = null)
        {
            ...
            var stockItemQuery = (
                ...
                select new ProductMasterAdvancedQuery()
                {
                    ...
                    MetalCategory = sac1.Description ?? "Unknown",
                    MetalType = sac2.Description ?? "Unknown",
